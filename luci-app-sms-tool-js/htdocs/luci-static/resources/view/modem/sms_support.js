...
function handleOpen(ev) {
	if (ev === 'ropenissues')      { window.open('https://github.com/4IceG/luci-app-sms-tool-js/issues'); return; }
	if (ev === 'copenissues')      { window.open('https://github.com/obsy/sms_tool/issues'); return; }
	if (ev === 'opendiscussion')   { window.open('https://github.com/4IceG/luci-app-sms-tool-js/discussions'); return; }
	...
	if (ev === 'opentopic')        { window.open('https://eko.one.pl/forum/viewtopic.php?id=20096&p=10'); return; }
}

return view.extend({
	load: function () {
		return uci.load('sms_tool_js');
	},
	...
	render: function (data) {
		...
		m = new form.Map('sms_tool_js', _('Package update and support'), info);

		s = m.section(form.TypedSection);
		s.anonymous = true;

		s.tab('info', _('Luci-app-sms-tool-js Info'));
		...
		let packages = [
			...
			{
				package: _('Luci-app-sms-tool-js'),
